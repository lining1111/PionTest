...
let sendData = () => {
  ...
  //设置发送进度条的最大值
  sendProgress.value.max = file.size;
  //设置接收进度条的最大值
  receiveProgress.value.max = file.size;
...
//接收消息处理
let onReceiveMessageCallback = (event:any) => {
  console.log(`接收的数据 ${event.data.byteLength}`);
  //将接收到的数据添加到接收缓存里
  receiveBuffer.push(event.data as never);
  //设置当前接收文件的大小
  ...
  if (receivedSize === file.size) {
    ...
    download.value.textContent = `点击下载'${file.name}'(${file.size} bytes)`;
